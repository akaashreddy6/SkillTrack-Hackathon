  ...
  return (
    <div className="home-page" id="top">
      ...
      <main className="home-main">
        ...
        <section className="home-section intelligence-section" id="intelligence"><div className="intelligence-copy"><span className="section-label">SKILL INTELLIGENCE</span><h2>Make the gap visible. Make the next step obvious.</h2><p>Personalized learning recommendations are generated from assessment performance, so every action is connected to a real capability signal.</p><Link to="/dashboard" className="text-link">Open your dashboard <span>→</span></Link></div><div className="intelligence-card">{snapshot.loading ? <div className="route-state">Loading your latest skill signal...</div> : leadSkill ? <><div className="intelligence-card-head"><span>{leadSkill.skills?.name}</span><small>Latest assessment</small></div><div className="intelligence-metrics"><div><strong>{score}%</strong><span>Current skill</span></div><div><strong>{target}%</strong><span>Target</span></div><div><strong>{gap}%</strong><span>Skill gap</span></div></div><div className="intelligence-track"><span style={{ width: `${score}%` }} /></div><p>Focus your next learning session on the gap with the highest impact.</p></> : <div className="empty-state">Complete an assessment to see your skill intelligence here.</div>}</div></section>
        <section className="home-section learning-preview"><div className="section-heading left"><span>PERSONALIZED LEARNING</span><h2>Learning that responds to where you are now.</h2></div><div className="learning-preview-grid"><div className="learning-plan-card"><div className="plan-top"><span className="section-label">YOUR PERSONALIZED LEARNING PLAN</span><span className="plan-status">{user ? "Live plan" : "Ready when you are"}</span></div><h3>{leadSkill?.skills?.name || "Your next skill"}</h3><p>{leadSkill ? "Recommended topics are prioritized from your latest assessment." : "Take an assessment to generate a learning plan built around your goals."}</p><div className="plan-values"><span>Current <b>{score !== undefined ? `${score}%` : "—"}</b></span><span>Target <b>{target !== undefined ? `${target}%` : "—"}</b></span><span>Gap <b>{gap !== undefined ? `${gap}%` : "—"}</b></span></div><Link to="/learning" className="button button-primary">View learning plan</Link></div><div className="topic-list"><span className="section-label">RECOMMENDED TOPICS</span>{["Fundamentals", "Functions", "Arrays", "DOM", "Async JavaScript"].map((topic, index) => <div key={topic}><span>0{index + 1}</span><strong>{topic}</strong><small>{leadSkill ? "Recommended" : "Assessment-led"}</small></div>)}</div></div></section>
        ...
        <section className="final-cta"><span className="section-label">YOUR NEXT STEP</span><h2>Ready to discover your next opportunity?</h2><Link to="/assessments" className="primary-button">Start Your Assessment <span>→</span></Link></section>
